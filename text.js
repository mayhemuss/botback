...
export const texts = {
  ...
  loteryRegDone: (name) => {
    return `Спасибо за регистрацию, ${
      ...
      `Важно: перед тем как они перейдут по ссылке, они должны подписаться на [SkyNetGames](https://t.me/games_skynet)\n\n` +
      `Минимальное количество приглашенных друзей - 2 человека!`
  },
  loteryRefUrl: (ref, callData) => {
    const RawReg = `${ref}_${callData}`
    return `Ваш друг хочет принять участие в Новогоднем розыгрыше от \"SkyNet GAMES\".\n\nЧто бы ваш друг смог принять в нем участие, вам сначала необходимо подписаться на ` +
      `[SkyNetGames](https://t.me/games_skynet), после чего перейти по реферальной ссылке:  [${BOT_NAME}?start=${
        codeText(RawReg)}] и ваш друг сможет принять участие в розыгрыше.`
    // }\n\n После перехода по ссылке откроется SkyNet GAMES BOT и в нем будет кнопка \"Зарегистрировать своего друга участником\".\n\n`+
    // `Перейдите по ней и введите свои данные, это необходимо для регистрации вашего друга участником розыгрыша.`
  },
  ...
  registrationDone: (commandName, gameName) => {
    return `Участники команды ${commandName}, поздравляем вас с успешной регистрацией команды на турнире по ${gameName}!\n` +
      `Ожидайте подтверждения.\n` +
      `ДЛЯ ПОДТВЕРЖДЕНИЯ УЧАСТИЯ С ВАМИ СВЯЖЕТСЯ АДМИН ЗА НЕСКОЛЬКО ДНЕЙ ДО ТУРНИРА!\n` +
